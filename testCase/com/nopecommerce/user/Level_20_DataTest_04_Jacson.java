...
import com.nopecommerce.data.UserDataMapper;
...
public class Level_20_DataTest_04_Jacson extends BaseTest{
...
//	DataHelper dataFake = DataHelper.getdataHelper();

	String userEmailAddress, userFirstName, userLastName, userPassword, userConfirmPassword;
	...
	public void beforeClass(String browserName) {
		...
		System.out.println(userData.getListSubjects().get(0).getName());
		userEmailAddress = userData.getEmailAddress() + getRandom() + "@gmai.com";
		 userFirstName =userData.getFirstName();
		 userLastName = userData.getLastName();
		 userPassword = userData.getLoginPassword();
		 userConfirmPassword = userData.getPassword();
		...
		userRegisterPage = PageGeneratorManager.getUserRegisterPageeObject(driver);
		
		log.info("beforeClass - Register to Portal User");
		userRegisterPage.RegisterToPortalUserByID(userFirstName, userLastName, userEmailAddress, userPassword, userConfirmPassword);
...
//		userHomePage = userRegisterPage.clickLogOutLink();
	...
	public void Role_01_User_To_Admin(Method method) {
		...
		userLoginPage = PageGeneratorManager.getUserLoginPageObject(driver);
		ExtentTestManager.getTest().log(Status.INFO,"Role_01_User_To_Admin - user login to portal user");
		userHomePage =  userLoginPage.LoginPotalUserPageByID(userEmailAddress, userPassword);
