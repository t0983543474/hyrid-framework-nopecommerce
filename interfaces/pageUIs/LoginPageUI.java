package pageUIs;

public class LoginPageUI {
	public static final String EMAIL_TXT = "//input[@id='Email']";
	public static final String PASSWORD_TXT = "//input[@id='Password']";
	public static final String LOGIN_BTN = "//button[@class='button-1 login-button']";
	
	public static final String MESSAGE_ERROR_TEXT = "//span[@id='Email-error']";
	public static final String MESSAGE_ERROR_SUMARY_TEXT ="//div[@class='message-error validation-summary-errors']";
	
}
